...
一個使用 SOCKS v4a 協議通過 Tor 代理進行網路連接的輕量級 C 語言工具。


## 專案簡介

Toralize 是一個簡單而有效的命令列工具，能夠通過本地 Tor SOCKS 代理（預設在 `127.0.0.1:9050`）將 TCP 連線匿名化。此工具實作了 SOCKS v4a 協議，讓您能夠透過 Tor 網路安全地連接到任何目標主機和端口。
